...
if ($_SERVER["REQUEST_METHOD"] != "POST" || !isset($_POST['inicio']) || !isset($_POST['fin']) || ($_POST['inicio'] > $_POST['fin'])) {
    ...
    class ReporteVentaFecha {
        ...
        public function __construct($id_venta, $fecha, $producto, $cantidad, $total) {
    ...
    $sql = "SELECT ventas.nro_comprobante AS id_venta, ventas.fecha_emision AS fecha, productos.descripcion AS producto, detalle_ventas.cantidad, (detalle_ventas.cantidad * detalle_ventas.precio) AS total
            ...
            WHERE ventas.fecha_emision BETWEEN '$inicio' AND '$fin'";    $result = $conn->query($sql);
    
    // Crear el arreglo para almacenar los objetos ReporteVentaFecha
    $ventas_por_fecha = [];

    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            // Crear una instancia de ReporteVentaFecha y agregarla al arreglo
            $ventas_por_fecha[] = new ReporteVentaFecha($row["id_venta"], $row["fecha"], $row["producto"], $row["cantidad"], $row["total"]);
        }
    }
...
<link rel="stylesheet" type="text/css" href="estilo_reportes_ventas_fecha.css">
...
<!-- linea recta 2 -->
    ...
    <table class="tabla_productos">
        ...
        <tbody>
            ...
            foreach($ventas_por_fecha as $reporte_venta)
            {
            ?>
                <tr>
                    ...
                    <td class="item_tabla body medio"><?php echo $reporte_venta->total; ?></td>
                    <!-- (modificar esto) -->
